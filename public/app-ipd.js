...
function loadYearConfig(be) {
  ...
  fetch(`/api/transfer-ipd/year-config/${be}`)
    .then(r => r.json())
    .then(data => {
      if (data.success) {
...
async function showData() {
  ...
  try {
    const res = await fetchPost('/api/transfer-ipd/preview', {
    ...
    document.getElementById('previewHead').innerHTML = `<tr>
      ...
      <th>#</th><th>HN</th><th>AN</th><th>วันที่-เวลาสแกน</th>
      <th>หน้า</th><th>รูปภาพ</th><th>ประเภท</th><th>Doc Type</th><th>เจ้าหน้าที่</th><th>สถานะ</th>
    </tr>`;

    // สร้าง selectedAns ติ๊กทุก row ตั้งต้น (ใช้ composite key an|doc_type)
    selectedAns = new Set(data.map(r => `${r.an}|${r.doc_type}`));
    ...
    if (data.length === 0) {
      ...
      data.forEach((row, i) => {
        ...
        const an = row.an || '';
        tbody.innerHTML += `
          <tr id="row-${an}-${row.doc_type}" class="row-selected">
            <td style="text-align:center">
              <input type="checkbox" class="row-check" data-key="${an}|${row.doc_type}" data-an="${an}" checked
                onchange="toggleRow('${an}|${row.doc_type}', this.checked)">
            </td>
            ...
            <td style="font-size:13px">${row.doc_type ?? '-'}</td>
            <td>${row.officer || '-'}</td>
...
function toggleRow(key, checked) {
  if (checked) selectedAns.add(key);
  else         selectedAns.delete(key);
  updateSelectedCount();
...
function toggleAllRows(checked) {
  document.querySelectorAll('.row-check').forEach(cb => {
    cb.checked = checked;
    if (checked) selectedAns.add(cb.dataset.key);
    else         selectedAns.delete(cb.dataset.key);
  });
  ['selectAllCheck', 'selectAllCheck2'].forEach(id => {
    const el = document.getElementById(id);
...
async function showTransferred() {
  ...
  try {
    const res = await fetchPost('/api/transfer-ipd/transferred', {
      startDate, endDate, year: currentConfigKey || selectedYear
    });
    ...
    showMsg('preview-msg', 'success',
      `✅ ต้นทางมี <strong>${sourceTotal.toLocaleString()}</strong> AN ในช่วงที่เลือก &nbsp;|&nbsp;
       พบในปลายทาง (ipt_chart_scan) แล้ว <strong>${total.toLocaleString()}</strong> รายการ`
    );
    ...
    if (data.length === 0) {
      ...
      data.forEach((row, i) => {
        tbody.innerHTML += `
          <tr>
            <td style="color:#94a3b8">${i + 1}</td>
            <td style="font-size:13px">${row.ipt_chart_scan_id ?? '-'}</td>
            <td>${row.an || '-'}</td>
            <td>${row.scan_datetime ? new Date(row.scan_datetime).toLocaleString('th-TH') : '-'}</td>
    ...
    document.getElementById('previewTitle').textContent = `รายการที่โอนแล้ว — ipt_chart_scan (พ.ศ. ${selectedYear})`;
    document.getElementById('previewTableWrap').style.display = 'block';
...
async function showOtherDates() {
  ...
  showMsg('preview-msg', 'loading', '<div class="spinner"></div> กำลังตรวจสอบ AN ที่ยังไม่ถูกโอน...');
  document.getElementById('previewTableWrap').style.display = 'none';
  document.getElementById('selectToolbar').style.display = 'none';

  try {
    const res = await fetchPost('/api/transfer-ipd/other-dates', {
      startDate, endDate, year: currentConfigKey || selectedYear
    });
    ...
    const { data, total, sourceTotal } = res;
    const missingAns = new Set(data.map(r => r.an)).size;

    showMsg('preview-msg', total === 0 ? 'success' : 'info',
      `📅 ต้นทางมี <strong>${sourceTotal}</strong> AN ในช่วงที่เลือก &nbsp;|&nbsp;
       ยังไม่ถูกโอน: <strong style="color:#d97706">${missingAns}</strong> AN (${total} รายการ)
       ${total === 0 ? '<br>✅ ทุก AN ถูกโอนไป ipt_chart_scan แล้ว' : ''}`
    );

    document.getElementById('previewHead').innerHTML = `<tr>
      <th>#</th><th>HN</th><th>AN</th><th>วันที่-เวลาสแกน</th>
      <th>หน้า</th><th>รูปภาพ</th><th>ประเภท</th><th>Doc Type</th><th>เจ้าหน้าที่</th>
    </tr>`;
    ...
    if (data.length === 0) {
      tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:#16a34a;padding:20px;font-weight:700">✅ ทุก AN ในช่วงนี้ถูกโอนไป ipt_chart_scan แล้ว</td></tr>';
    } else {
      data.forEach((row, i) => {
        const hasImg = row.has_image == 1;
        tbody.innerHTML += `
          <tr style="background:#fef9c3">
            <td style="color:#94a3b8">${i + 1}</td>
            <td><strong>${row.hn || '-'}</strong></td>
            <td style="color:#d97706;font-weight:700">${row.an || '-'}</td>
            <td>${row.scan_date_time || '-'}</td>
            <td style="text-align:center">${row.pageno ?? '-'}</td>
            <td>${hasImg ? '<span class="badge-has">มีข้อมูล</span>' : '<span class="badge-no">ไม่มีข้อมูล</span>'}</td>
            <td>${row.image_type || '-'}</td>
    ...
    document.getElementById('previewTitle').textContent = `AN ที่ยังไม่ถูกโอน — ipt_chart_scan (พ.ศ. ${selectedYear})`;
    document.getElementById('previewTableWrap').style.display = 'block';
  } catch (e) {
    showMsg('preview-msg', 'error', '❌ ' + e.message);
  }
...
async function startTransfer() {
  ...
  try {
    const res = await fetchPost('/api/transfer-ipd/start', {
      startDate, endDate, year: currentConfigKey || selectedYear,
      selectedKeys: [...selectedAns]   // format: "an|doc_type"
    });
...
function pollProgress(jobId) {
  const interval = setInterval(async () => {
    try {
      const res = await fetch(`/api/transfer-ipd/status/${jobId}`);
      const data = await res.json();
      if (data.success) {
        updateProgressUI(data);
